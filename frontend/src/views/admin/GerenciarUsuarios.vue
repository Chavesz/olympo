...
const form = ref({ id: '', nome: '', email: '', role: 'aluno', status: 'ativo' })
...
async function onSave() {
  ...
  try {
    if (isEditing.value) {
      const updated = await api.patch(`/usuarios/${form.value.id}`, {
      ...
      const created = await api.post('/usuarios', {
        nome: form.value.nome.trim(),
        email: normalizeEmail(form.value.email),
        senha: '123456',
        role: form.value.role,
...
async function onToggleStatus(u) {
  ...
  const updated = await api.patch(`/usuarios/${u.id}`, { status: novoStatus })
...
<template>
  <section class="space-y-6">
    <header class="space-y-1">
      ...
      <p class="text-sm text-slate-700">Usuários cadastrados no banco de dados.</p>
    </header>
            ...
            <tr v-for="u in filteredUsers" :key="u.id" class="hover:bg-slate-50">
              ...
              <td class="px-4 py-3">
                <span
                  ...
                  :class="u.role === 'admin' ? 'bg-indigo-50 text-indigo-700' : u.role === 'instrutor' ? 'bg-sky-50 text-sky-700' : 'bg-slate-100 text-slate-700'"
                >{{ u.role }}</span>
              </td>
              <td class="px-4 py-3">
                <button
              ...
              <td class="px-4 py-3">
                <div class="flex justify-end gap-2">
                  <button class="rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-sm font-semibold text-slate-900 hover:bg-slate-50" type="button" @click="openEdit(u)">Editar</button>
                  <button class="rounded-lg border border-rose-300 bg-rose-50 px-3 py-1.5 text-sm font-semibold text-rose-700 hover:bg-rose-100" type="button" @click="onDelete(u)">Excluir</button>
                </div>
              </td>
            </tr>
          ...
          <div class="w-full rounded-2xl border border-slate-200 bg-white p-5 text-slate-900 shadow-xl" role="dialog" aria-modal="true">
            <div class="flex items-start justify-between gap-3">
              <div>
                ...
                <div class="text-sm text-slate-700">{{ isEditing ? 'Atualize os dados.' : 'Senha padrão: 123456' }}</div>
              </div>
            ...
            <div class="mt-4 space-y-3">
              <label class="block space-y-1">
                ...
                <input v-model="form.nome" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none ring-slate-400 focus:ring-2" type="text" />
              </label>
              <label class="block space-y-1">
                <span class="text-sm font-medium text-slate-800">E-mail</span>
                <input v-model="form.email" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none ring-slate-400 focus:ring-2" type="email" />
              </label>
              <div class="grid gap-3 sm:grid-cols-2">
                <label class="block space-y-1">
                  <span class="text-sm font-medium text-slate-800">Papel</span>
                  <select v-model="form.role" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none ring-slate-400 focus:ring-2">
