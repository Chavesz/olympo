...
const form = ref({ id: '', usuario_id: '', nome: '', email: '', telefone: '', status: 'ativo' })

onMounted(async () => {
  loading.value = true
  alunos.value = await api.get('/alunos')
  loading.value = false
})
...
function openCreate() {
  ...
  form.value = { id: '', usuario_id: '', nome: '', email: '', telefone: '', status: 'ativo' }
...
async function onSave() {
  ...
  try {
    if (isEditing.value) {
      const updated = await api.patch(`/alunos/${form.value.id}`, {
        telefone: form.value.telefone,
        status: form.value.status,
      })
      alunos.value = alunos.value.map(a => a.id === updated.id ? updated : a)
    } else {
      // Criar usuário com role aluno — o backend cria o perfil de aluno junto
        ...
        <table class="min-w-full text-left text-sm">
          ...
          <tbody class="divide-y divide-slate-200">
            <tr v-for="a in filteredAlunos" :key="a.id" class="hover:bg-slate-50">
              ...
              <td class="px-4 py-3 text-slate-700">{{ a.plano_ativo || '-' }}</td>
              <td class="px-4 py-3">
                <button
          ...
          <div class="w-full rounded-2xl border border-slate-200 bg-white p-5 text-slate-900 shadow-xl" role="dialog" aria-modal="true">
            <div class="flex items-start justify-between gap-3">
              <div>
                ...
                <div class="text-sm text-slate-700">{{ isEditing ? 'Atualize telefone e status.' : 'Senha padrão: 123456' }}</div>
              </div>
