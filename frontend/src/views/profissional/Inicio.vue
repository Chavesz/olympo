...
import { loadAulas, loadComunicados, loadEventos } from '../../services/mockPortal'

const alunosAtivos = computed(() => loadAlunos().filter((a) => a.status === 'ativo').length)

const eventosAbertos = computed(() => {
  const now = new Date().toISOString().slice(0, 10)
  return loadEventos().filter((e) => e.data >= now).length
})

const aulasSemana = computed(() => {
  const aulas = loadAulas()
  return aulas.reduce((acc, m) => acc + (Array.isArray(m.slots) ? m.slots.length : 0), 0)
})

const comunicadosRecentes = computed(() => loadComunicados().slice(0, 3))
</script>
