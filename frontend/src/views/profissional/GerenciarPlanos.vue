...
function onSave() {
  ...
  const id =
    form.value.id || (typeof crypto?.randomUUID === 'function' ? crypto.randomUUID() : `p-${Date.now()}`)
  ...
  plans.value = upsertPlan(payload)
  closeModal()
}
...
function onDelete(p) {
  ...
  if (inUse) {
    plans.value = upsertPlan({ ...p, status: 'inativo' })
    return
  }

  plans.value = deletePlan(p.id)
}
</script>
...
<template>
  <section class="space-y-6">
    <header class="space-y-1">
      ...
      <p class="text-sm text-slate-700">Crie, edite e desative planos (mock).</p>
    </header>
