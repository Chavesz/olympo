...
import { loadAulas, loadSubscriptions, saveAulas, saveSubscriptions } from '../../services/mockPortal'
...
function isSubscribed(slotId) {
  return subs.value.some((s) => s.type === 'aula' && String(s.id).startsWith(`${slotId}|`))
}
...
function subscribe(modalidade, slot) {
  if (!userId.value) return
  if (isSubscribed(slot.id)) return
  if (isFull(slot)) return

  const subId = `${slot.id}|${modalidade}`
  subs.value = [{ type: 'aula', id: subId }, ...subs.value]
  ...
  aulas.value = aulas.value.map((m) => {
    ...
    return {
      ...m,
      slots: m.slots.map((s) => (s.id === slot.id ? { ...s, inscritos: s.inscritos + 1 } : s)),
    }
  })
        ...
        <div class="grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-3">
          <div v-for="s in m.slots" :key="s.id" class="rounded-lg border border-slate-200 bg-slate-50 p-3">
            ...
            <button
              ...
              @click="subscribe(m.modalidade, s)"
            >
