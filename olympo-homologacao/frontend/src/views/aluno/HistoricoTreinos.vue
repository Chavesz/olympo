...
const sessoes = computed(() =>
  history.value
    .slice()
    .sort((a, b) => String(b.dataTreino).localeCompare(String(a.dataTreino))),
)
    ...
    <div class="overflow-hidden rounded-xl border border-slate-200 bg-white">
      ...
      <ul class="divide-y divide-slate-200">
        <li v-for="h in sessoes" :key="h.id" class="flex items-center justify-between gap-3 px-4 py-4">
          <div>
            <div class="text-sm font-semibold text-slate-900">{{ h.fichaTitulo || 'Treino' }}</div>
            <div class="text-sm text-slate-700">{{ fmtDate(h.dataTreino) }}</div>
          </div>
