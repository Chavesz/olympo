...
async function refresh() {
  const [u, a, i] = await Promise.all([fetchUsers(), fetchAlunos(), fetchInstrutores()])
  users.value = u
  alunos.value = a
  instrutores.value = i
}
...
function userPayload() {
  return {
    ...
    cref: form.value.cref.trim() || undefined,
    especialidade: form.value.especialidade.trim() || undefined,
  }
}
...
async function onSave() {
  ...
  try {
    if (isEditing.value) {
      await updateUser(form.value.id, userPayload())
    } else {
      await createUser({ ...userPayload(), senha: '123456' })
    }
    await refresh()
    closeModal()
  } catch (e) {
    formError.value = e.message || 'Erro ao salvar usuário.'
  }
}
...
async function onDelete(u) {
  ...
  try {
    ...
    window.alert(e.message || 'Erro ao excluir usuário.')
  }
}
