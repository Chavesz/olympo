...
onMounted(async () => {
  try {
    unidades.value = await loadUnidades()
  } finally {
    loading.value = false
  }
})
...
function openEdit(u) {
  ...
  form.value = {
    ...
    telefone: u.telefone,
    horario: u.horario,
    imagemKey: u.imagemKey ?? 'centro',
    planoMensal: u.planoMensal,
    planoAnual: u.planoAnual,
  }
  isModalOpen.value = true
}
...
async function onDelete(u) {
  ...
  try {
    unidades.value = await deleteUnidade(u.id)
  } catch (e) {
    window.alert(e.message || 'Erro ao excluir unidade.')
  }
}
        ...
        <div class="relative mx-auto flex min-h-full max-w-md items-center px-4 py-8">
          <div class="max-h-[90vh] w-full overflow-y-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-xl" role="dialog" aria-modal="true">
            ...
            <div class="mt-4 space-y-3">
              ...
              <label class="block space-y-1">
                <span class="text-sm font-medium">Imagem da unidade</span>
                <select v-model="form.imagemKey" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm">
