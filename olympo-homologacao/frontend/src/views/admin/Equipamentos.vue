...
onMounted(async () => {
  try {
    const [eq, un] = await Promise.all([loadEquipamentos(), loadUnidades()])
    equipamentos.value = eq
    unidades.value = un
  } finally {
    loading.value = false
  }
...
async function onSave() {
  ...
  try {
    equipamentos.value = await upsertEquipamento({
    ...
    formError.value = e.message || 'Erro ao salvar equipamento.'
  }
}
...
async function onDelete(e) {
  ...
  try {
    equipamentos.value = await deleteEquipamento(e.id)
  } catch (err) {
    window.alert(err.message || 'Erro ao excluir equipamento.')
  }
}
</script>
