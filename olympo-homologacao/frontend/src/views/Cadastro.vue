...
function validate() {
  ...
  if (perfil.value === 'instrutor') return 'Cadastro de instrutor deve ser feito pelo administrador.'
  return ''
}
...
async function onSubmit() {
  ...
  try {
    ...
    errorMessage.value = e.message || 'Não foi possível concluir o cadastro. Tente novamente.'
  } finally {
    ...
    <div class="w-full max-w-md space-y-6">
      ...
      <form class="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm" @submit.prevent="onSubmit">
        <header class="space-y-1 text-center">
          ...
          <p class="text-sm text-slate-600">Registre um novo usuário.</p>
        </header>
