...
async function refresh() {
  const [ev, ins] = await Promise.all([fetchEventos(), fetchTodasInscricoes()])
  eventos.value = ev
  todasInscricoes.value = ins
}
...
async function onSave() {
  ...
  try {
    if (isEditing.value && form.value.id) {
      await updateEvento(form.value.id, payload)
    } else {
    ...
    formError.value = e.message || 'Erro ao salvar evento.'
  }
}
...
async function onDelete(e) {
  ...
  try {
    ...
    window.alert(err.message || 'Erro ao excluir evento.')
  }
}

function openInscritos(e) {
  inscritosEvento.value = e
  inscritosLista.value = todasInscricoes.value
    .filter((row) => row.tipo === 'evento' && row.evento_id === e.id)
    .map((row) => ({
      id: row.id,
      nome: row.usuario_nome ?? 'Aluno',
    }))
...
<template>
  <section class="space-y-6">
    <header class="space-y-1">
      ...
      <p class="text-sm text-slate-700">Crie, edite e arquive eventos.</p>
    </header>
