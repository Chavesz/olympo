...
async function refresh() {
  comunicados.value = await fetchComunicados()
}
...
async function onSave() {
  ...
  try {
    if (isEditing.value && form.value.id) {
      await updateComunicado(form.value.id, payload)
    } else {
    ...
    formError.value = e.message || 'Erro ao salvar comunicado.'
  }
}
...
async function onDelete(c) {
  ...
  try {
    ...
    window.alert(e.message || 'Erro ao excluir comunicado.')
  }
}
</script>
