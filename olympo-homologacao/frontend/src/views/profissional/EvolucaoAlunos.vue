...
import { loadEvaluations, saveEvaluation } from '../../services/evaluations'
...
onMounted(async () => {
  ...
  if (alunos.value.length) selectedAlunoId.value = alunos.value[0].id
})

async function loadItems(alunoId) {
  if (!alunoId) {
    items.value = []
    return
  }
  items.value = await loadEvaluations(alunoId)
}
...
async function onSave() {
  ...
  if (isEditing.value) {
    formError.value = 'Edição de avaliações não disponível via API.'
    return
  }
  ...
  try {
    await saveEvaluation(selectedAlunoId.value, payload)
    await loadItems(selectedAlunoId.value)
    closeModal()
  } catch (e) {
...
function onDelete() {
  formError.value = 'Exclusão de avaliações não disponível via API.'
}
</script>
